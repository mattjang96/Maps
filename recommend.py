...
def find_closest(location, centroids):
    ...
    """
    # BEGIN Question 3
    return min([k for k in centroids], key = lambda x: distance(x, location))
    # END Question 3
...
def group_by_centroid(restaurants, centroids): #use group_by_first
...
def find_centroid(cluster):
    ...
    # BEGIN Question 5
    places = [restaurant_location(r) for r in cluster]
    return[mean([x[0] for x in places]), mean([x[1] for x in places])]
    # END Question 5


def k_means(restaurants, k, max_updates=100): #hint: use find_centroid
    ...
    while old_centroids != centroids and n < max_updates:
        ...
        centroids = [find_centroid(x) for x in groups if len(x)!=0]
        # END Question 6
        n += 1
    return centroids
...
def find_predictor(user, restaurants, feature_fn):
    ...
    b, a = s_xy/s_xx, y_mean-s_xy/s_xx*x_mean  # REPLACE THIS LINE WITH YOUR SOLUTION
...
def best_predictor(user, restaurants, feature_fns):
    ...
    # BEGIN Question 8
    return max([find_predictor(user, reviewed, a) for a in feature_fns], key = lambda x: x[1])[0]
    # END Question 8
...
def rate_all(user, restaurants, feature_fns):
    ...
    for i in restaurants:
        n = restaurant_name(i)
        if i in reviewed:
            ratings[n] = user_rating(user, n)
        else:
            ratings[n] = predictor(i)
    return ratings
    # END Question 9
